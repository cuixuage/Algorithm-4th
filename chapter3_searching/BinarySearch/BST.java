package chapter3_searching.BinarySearch;
import chapter3_searching.BalancedSearchTree.RedBlackBST;
...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    private Node put(Node x, Key key, Value val) {
        ...
        int cmp = key.compareTo(x.key);
        //TODO 寻找合适插入位置(新节点都会放在最后一层leaf)
        if      (cmp < 0) x.left  = put(x.left,  key, val);
        else if (cmp > 0) x.right = put(x.right, key, val);
        else              x.val   = val;            //key已经存在 则只更新
    ...
    private Node floor(Node x, Key key) {
        ...
        int cmp = key.compareTo(x.key);
        if (cmp == 0) return x;
        //root > key 那么小于等于key的最大值在root.left
        if (cmp <  0) return floor(x.left, key);
        //否则 小于等于key的最大值为root 或者 root.right
        Node t = floor(x.right, key);
        if (t != null) return t;
        else return x;  //向下取整最大值为root
    }
